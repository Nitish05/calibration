...
def generate_gcode(offset_pts, feed_rate=1000.0, heading_deg=None):
    ...
    lines = [
        ...
        "G21 ; mm",
        "G90 ; absolute",
        f"G0 X{offset_pts[0, 0]:.2f} Y{offset_pts[0, 1]:.2f}{_a(0)} ; rapid to start",
    ]
    for i, pt in enumerate(offset_pts[1:], start=1):
        lines.append(f"G1 X{pt[0]:.2f} Y{pt[1]:.2f}{_a(i)} F{feed_rate:.0f}")
    lines.append(f"G1 X{offset_pts[0, 0]:.2f} Y{offset_pts[0, 1]:.2f}{_a(0)} ; close loop")
    lines.append("M2")
    return "\n".join(lines)
