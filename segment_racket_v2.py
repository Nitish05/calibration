...
if args.headless:
    ...
    def gcode_endpoint():
        ...
        gcode = generate_gcode(pts, feed_rate=args.feed_rate)
        # Also save to disk
...
def generate_gcode(offset_pts, feed_rate=1000.0):
    """Generate G-code for a closed XY-only path (no Z commands)."""
    lines = [
        "; Racket offset path — generated by segment_racket_v2.py",
        f"; {len(offset_pts)} points",
        "G21 ; mm",
        "G90 ; absolute",
        f"G0 X{offset_pts[0, 0]:.2f} Y{offset_pts[0, 1]:.2f} ; rapid to start",
    ]
    for pt in offset_pts[1:]:
        lines.append(f"G1 X{pt[0]:.2f} Y{pt[1]:.2f} F{feed_rate:.0f}")
    lines.append(f"G1 X{offset_pts[0, 0]:.2f} Y{offset_pts[0, 1]:.2f} ; close loop")
    lines.append("M2")
    return "\n".join(lines)
    ...
    while True:
        ...
        if args.headless:
            ...
            if key == ord('g'):
                ...
                if gpts is not None:
                    gcode = generate_gcode(gpts, feed_rate=args.feed_rate)
                    with open(args.gcode_output, "w") as f:
                        f.write(gcode)
